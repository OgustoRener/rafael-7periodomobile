...
export default function ProdutoListScreen({ navigation }) {
  ...
  const carregarProdutos = async () => {
    try {
      ...
      abrirModal('erro', 'Não foi possível carregar a lista de produtos.')
    } finally {
      setLoading(false)
    }
  }
...
const styles = StyleSheet.create({
  ...
  titulo: { fontSize: 30, fontWeight: '800', color: '#0f172a' },
  subtitulo: { fontSize: 14, color: '#94a3b8', marginTop: 2 },
  botaoNovo: {
    width: 48, height: 48, borderRadius: 14, backgroundColor: '#7c3aed',
    justifyContent: 'center', alignItems: 'center',
    shadowColor: '#7c3aed', shadowOpacity: 0.4, shadowRadius: 8,
    shadowOffset: { width: 0, height: 4 }, elevation: 4,
  },
  lista: { paddingBottom: 24 },
  listaVazia: { flexGrow: 1, justifyContent: 'center' },
  vazioContainer: { alignItems: 'center', gap: 8 },
  textoVazio: { textAlign: 'center', color: '#64748b', fontSize: 16, fontWeight: '600', marginTop: 8 },
  textoVazioSub: { textAlign: 'center', color: '#94a3b8', fontSize: 14 },
})